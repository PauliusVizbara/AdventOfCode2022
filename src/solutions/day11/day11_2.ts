...
class Monkey {
    ...
    throwItems(): ThrownItem[] {
        ...
        while (this.items.length) {
            this.inspectedTimes++
            const item = this.executeOperation(this.items.shift()) % this.lcm
...
export default async () => {
    ...
    const monkeyDivisors = monkeys.map(m => m.testDivisor)
    const divisorLCM = monkeyDivisors.reduce(lcm)

    console.log(divisorLCM);
    monkeys.forEach(monkey => {
        monkey.lcm = divisorLCM
    });
    ...
    for (let i = 0; i < rounds; i++) {
        monkeys.forEach(monkey => {
            ...
            thrownItems.forEach(thrownItem => {
                monkeys[thrownItem.recipient].items.push(thrownItem.item)
            })
        })
    }
